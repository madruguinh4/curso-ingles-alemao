...
export function Review() {
  ...
  useEffect(() => {
    ...
    dueCards(eid).then((c) => {
      setQueue(c)
      if (!c.length) nextDue(eid).then(setNext)
    })
  }, [eid])

  useEffect(() => {
    if (queue && pos >= queue.length && queue.length > 0) markReviewDone(eid)
  }, [queue, pos, eid])
  ...
  if (queue.length === 0) {
    return (
      <Screen title={`Revisão — ${LANGUAGE_NAMES[lang]}`}>
        <Card>
          <p className="text-lg font-semibold">Nada para revisar hoje.</p>
  ...
  if (pos >= queue.length) {
    return (
      <Screen title="Revisão concluída">
        <Card>
          <p className="text-lg">Você revisou <b>{queue.length}</b> cartão(ões) de {LANGUAGE_NAMES[lang]}.</p>
          <p className="muted mt-1">Cada um foi reagendado conforme a sua resposta.</p>
        </Card>
  ...
  return (
    <Screen title={`Revisão — ${LANGUAGE_NAMES[lang]}`}>
      ...
      {!v ? (
        <Notice kind="warn">Cartão de um vocabulário que não existe mais no conteúdo ({card.vocabId}). <Button variant="ghost" onClick={() => grade('good')}>Pular</Button></Notice>
      ) : (
        <Card className="mt-3">
          <p className="text-sm muted">{flipped ? 'Resposta' : 'Você lembra o que significa?'}</p>
          <p className="text-3xl font-bold my-3">{vocabFront(v, lang)} <Say text={v.term} lang={lang} /></p>
          {flipped ? (
            <div className="grid gap-2">
              <p className="text-xl">{v.translation}</p>
              <p><i>{v.example}</i> <Say text={v.example} lang={lang} /></p>
              <p className="text-sm muted">{v.exampleTranslation}</p>
              {related && (
              ...
              <div className="grid grid-cols-4 gap-2 mt-3">
                {(['again', 'hard', 'good', 'easy'] as Grade[]).map((g) => (
                  <Button key={g} variant={g === 'again' ? 'secondary' : g === 'good' ? 'primary' : 'secondary'} onClick={() => grade(g)}>{GRADE_LABELS[g]}</Button>
                ))}
              </div>
              <p className="text-xs muted">Errei = volta logo · Difícil = intervalo curto · Bom = intervalo normal · Fácil = intervalo longo</p>
            </div>
          ) : (
...
async function markReviewDone(eid: number) {
  const item = await db.schedule.where('[enrollmentId+date]').equals([eid, todayISO()]).filter((i) => i.kind === 'review' && i.status === 'pending').first()
